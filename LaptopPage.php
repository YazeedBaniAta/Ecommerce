...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    if (isset($_POST['LaptopDevice_submit'])) {
...
<div class="container-fluid mb-5 mt-3">
     <div class="row">
         <div class="col-lg-3">
			 ...
			 <hr>
		     <h5 class="text-secondary font-weight-bold" style="font-size: 1.2rem; font-family: 'Bebas Neue', cursive;">Price</h5>
             <ul class="list-group">
                 <?php
                 $statement = "SELECT DISTINCT(product_new_price) FROM all_products WHERE product_type = 'L' ORDER BY product_new_price DESC";
                 ...
                 while ($row=$result->fetch_assoc()){
                     ...
                     <li class="list-group-item">
                         <div class="form-check">
                             <label >
                                 <input type="checkbox" class="Price product_check_L" value="<?php echo $row['product_new_price']; ?>" > <?php echo $row['product_new_price']; ?>
                             </label>
                         </div>
                     </li>
                 <?php }?>
            ...
            <h5 class="text-secondary font-weight-bold" style="font-size: 1.2rem; font-family: 'Bebas Neue', cursive;">Brand</h5>
               <ul class="list-group">
				 <?php
				    $statement = "SELECT DISTINCT(product_brand) FROM all_products WHERE product_type = 'L' ORDER BY product_brand DESC";
				    $result = $conn->query($statement);
				    while ($row=$result->fetch_assoc()){
                  ?>
                   <li class="list-group-item">
                     <div class="form-check">
                        <label >
                        <input type="checkbox" class="brand product_check_L" value="<?php echo $row['product_brand']; ?>" > <?php echo $row['product_brand']; ?>
                        </label>
                     </div>
                    </li>
                  <?php }?>
         ...
         <!-- For display all RAM from DataBase -->
            <h5 class="text-secondary font-weight-bold" style="font-size: 1.2rem; font-family: 'Bebas Neue', cursive;">RAM</h5>
               <ul class="list-group">
				 <?php
				    $statement = "SELECT DISTINCT(product_ram) FROM all_products WHERE product_type = 'L' ORDER BY product_ram DESC";
				    $result = $conn->query($statement);
				    while ($row=$result->fetch_assoc()){
                  ?>  
                   <li class="list-group-item">
                     <div class="form-check">
                        <label >
                        <input type="checkbox" class="ram product_check_L" value="<?php echo $row['product_ram']; ?>" > <?php echo $row['product_ram']; ?> GB
                        </label>
                     </div>
                    </li>
                  <?php }?>
             ...
             <!-- For display all Internal Storage from DataBase -->
            <h5 class="text-secondary font-weight-bold" style="font-family: 'Bebas Neue', cursive;">Internal Storage</h5>
               <ul class="list-group">
				 <?php
				    $statement = "SELECT DISTINCT(product_storage) FROM all_products WHERE product_type = 'L' ORDER BY product_storage DESC";
				    $result = $conn->query($statement);
				    while ($row=$result->fetch_assoc()){
                  ?>  
                   <li class="list-group-item">
                     <div class="form-check">
                        <label >
                        <input type="checkbox" class="storage product_check_L" value="<?php echo $row['product_storage']; ?>"> <?php echo $row['product_storage']; ?> GB
                        </label>
                     </div>
                    </li>
                  <?php }?>
          ...
          <div class="col-lg-9">
            <h4 class="text-info font-weight-bold" style="font-family: 'Dela Gothic One', cursive;">Laptop Device</h4>
            <hr>
			 <div class="text-center">
			   <img class="img-fluid mt-5" alt="loder" src="img/products/Spinner.gif" id="loder_P" style="height: 150px; display:none; ">
			 </div> 
			 <div class="row" id="result_L">
			   <?php 
			     $statement = "SELECT * FROM all_products WHERE product_type = 'L'";
			     $result = $conn->query($statement);
			     while ($row=$result->fetch_assoc()){
			   ?>
			     ...
			     <div class="item border border-success bg-light rounded">
			      <div class="product">
			        ...
			        <div class="mt-2 ml-2">
			         ...
			         <p>
			           Ram : <?php echo $row['product_ram'];?> GB <br>
			           Storage : <?php echo $row['product_storage'];?> GB <br>
			           product_processor : <?php echo $row['product_processor'];?> <br>
			           product_screen_size : <?php echo $row['product_screen_size'];?> <br>
			         </p>
			        </div>
			        <div class="text-center mb-2">
                        <form method="post">
                            ...
                            if (in_array($row['product_id'], $Cart->getCartId($Proudects->getData('cart')) ?? [])){
                                ...
                                echo '<button type="submit" name="LaptopDevice_submit" class="btn btn-warning font-size-12 add_to_cart">Add to Cart</button>';
                            }
                            ?>
