...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    if (isset($_POST['MobileDevice_submit'])) {
...
<div class="container-fluid mb-5 mt-3">
     <div class="row">
         <div class="col-lg-3">                				
			 <h4class="text-secondary font-weight-bold" style="text-size:1.6rem; font-family: 'Lobster', cursive;">Filter Products</h4>
			 <hr>
          ...
          <!-- For display all Brand from DataBase -->
            <h5 class="text-secondary font-weight-bold" style="font-size: 1.2rem; font-family: 'Bebas Neue', cursive;">Brand</h5>
               <ul class="list-group">
				 <?php
				    $statement = "SELECT DISTINCT(product_brand) FROM all_products WHERE product_type = 'M' ORDER BY product_brand DESC";
                  ...
                  ?>  
                   <li class="list-group-item">
                     <div class="form-check">
                        <label >
                        <input type="checkbox" class="brand product_check" value="<?php echo $row['product_brand']; ?>" > <?php echo $row['product_brand']; ?>
                        </label>
                     </div>
                    </li>
                  <?php }?>
         ...
         <!-- For display all RAM from DataBase -->
            <h5 class="text-secondary font-weight-bold" style="font-size: 1.2rem; font-family: 'Bebas Neue', cursive;">RAM</h5>
               <ul class="list-group">
				 <?php
				    $statement = "SELECT DISTINCT(product_ram) FROM all_products WHERE product_type = 'M' ORDER BY product_ram DESC";
				    $result = $conn->query($statement);
				    while ($row=$result->fetch_assoc()){
                  ?>  
                   <li class="list-group-item">
                     <div class="form-check">
                        <label >
                        <input type="checkbox" class="ram product_check" value="<?php echo $row['product_ram']; ?>" > <?php echo $row['product_ram']; ?> GB
                        </label>
                     </div>
                    </li>
                  <?php }?>
            ...
            <h5 class="text-secondary font-weight-bold" style="font-size: 1.2rem; font-family: 'Bebas Neue', cursive;">Internal Storage</h5>
               <ul class="list-group">
				 <?php
				    $statement = "SELECT DISTINCT(product_storage) FROM all_products WHERE product_type = 'M' ORDER BY product_storage DESC";
				    $result = $conn->query($statement);
				    while ($row=$result->fetch_assoc()){
                  ?>  
                   <li class="list-group-item">
                     <div class="form-check">
                        <label >
                        <input type="checkbox" class="storage product_check" value="<?php echo $row['product_storage']; ?>"> <?php echo $row['product_storage']; ?> GB
                        </label>
                     </div>
                    </li>
                  <?php }?>
          ...
          <div class="col-lg-9">
            <h4 class="text-info font-weight-bold" style="font-family: 'Dela Gothic One', cursive;">Mobile Phone</h4>
            <hr>
			 ...
			 <div class="row" id="result">
			   <?php 
			     $statement = "SELECT * FROM all_products WHERE product_type = 'M'";
			     $result = $conn->query($statement);
			     while ($row=$result->fetch_assoc()){
			   ?>
			        ...
			        <div class="text-center mb-2">
                        <form method="post">
                            ...
                            if (in_array($row['product_id'], $Cart->getCartId($Proudects->getData('cart')) ?? [])){
                                ...
                                echo '<button type="submit" name="MobileDevice_submit" class="btn btn-warning font-size-12 add_to_cart">Add to Cart</button>';
                            }
                            ?>
